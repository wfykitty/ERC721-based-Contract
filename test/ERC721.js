...
contract("ERC721", (accounts) => {
    ...
    it("test: transferFrom()", async () => {
        ...
        const [meAsAdminBalance, user1Balance, owner] = await Promise.all([instance.balanceOf(meAsAdmin), instance.balanceOf(user1), instance.ownerOf(tokenId)]
        );
    
        assert(new BigNumber(meAsAdminPreBalance).minus(new BigNumber(meAsAdminBalance)).isEqualTo(new BigNumber(user1Balance)), 
        "not correct amount"
        );
    ...
    truffleAssert.eventEmitted(receipt, "Transfer", (obj) => {
        return (
        obj.from === meAsAdmin && obj.to === user1 &&
    ...
    it("test: safeTransferFrom()", async () => {
        ...
        assert(meAsAdminBalance.toNumber() === 1, "This is not the expected supply");
        assert(owner === user1, "This is not the expected owner");

        truffleAssert.eventEmitted(receipt, "Transfer", (obj) => {
            return (obj._from === me && obj._to === user1 && 
    ...
    it("Test: setApprovalForAll()", async () => {
        const receipt = await instance.setApprovalForAll(meAsAdmin, true, {from:user1});
        
        truffleAssert.eventEmitted(receipt, "ApprovalForAll", (obj) => {
        return (obj._owner === user1 && obj._operator === meAsAdmin && obj._approved === true);
            });
...
contract("Planet", (accounts) => {
    ...
    const firstPlanetLevel = new BigNumber(1);
    const firstPlanetResource = new BigNumber(10);
    ...
    it("test: the first planet has been created: Marvelstar", async () => {
        const tokenId = 0;
        const receipt = await planetInstance.planetkey(tokenId);
    
        assert(
        receipt.name === firstPlanetName, "This is not expected planet Name"
        );
        assert(
        new BigNumber(receipt.level).isEqualTo(firstPlanetLevel), "This is not expected level of planet."
        );
        assert(
        new BigNumber(receipt.resource).isEqualTo(firstPlanetResource), "This is not expected resource of planet."
        );
    });
    ...
    const resource = 10;
    const planetId = 1;
    const planetReceipt = await planetInstance.newPlanet(
        name, level, resource, {from: user1});
    const mintedTx = await planetInstance.mint(user1, planetId);

    truffleAssert.eventEmitted(mintedTx, "Transfer", (obj) => {
    return (obj._from === mintAddress && obj._to === user1 && 
        new BigNumber(obj._tokenId).isEqualTo(new BigNumber(planetId)));
      });
    assert(planetReceipt, name, level, resource, "This is not the expected information");
    ...
    it("Test: get the singlePlanet using ID", async () => {
        ...
        assert(new BigNumber(receipt[1]).isEqualTo(new BigNumber(1)), "This is not expected level");
        assert(new BigNumber(receipt[2]).isEqualTo(new BigNumber(10)), "This is not expected resource");
